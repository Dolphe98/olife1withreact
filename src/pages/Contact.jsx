...
export default function Contact() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-10 md:pt-20 pb-10 space-y-24 md:space-y-32">
      ...
      {/* SECTION 2: WHATSAPP COMMUNITY (SEO OPTIMIZED) */}
      <div className="flex flex-col md:flex-row items-center gap-12 bg-slate-900/20 border border-slate-800 p-8 md:p-16 rounded-3xl shadow-2xl">
        <div className="flex-1 space-y-8">
          ...
          <div className="flex flex-col gap-4">
            <a 
                href="https://chat.whatsapp.com/H3j7yRTaKIvLFA8xNNjJ2S" 
                target="_blank" 
                rel="noopener noreferrer nofollow" 
                aria-label="Join Olife WhatsApp Group"
            ...
            <a 
                href="https://whatsapp.com/channel/0029VbChk337oQhilYhgdA0Z" 
                target="_blank" 
                rel="noopener noreferrer nofollow" 
                aria-label="Follow Olife WhatsApp Channel"
      ...
      <div className="max-w-3xl mx-auto space-y-12">
        ...
        <form onSubmit={onContactSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label htmlFor="first_name" className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-2">First Name</label>
            <input id="first_name" type="text" name="First_Name" required className="w-full bg-slate-900/50 border border-slate-800 rounded-lg p-4 text-slate-200 focus:outline-none focus:border-cyan-600 transition-colors" placeholder="John" />
          </div>
          <div className="space-y-2">
            <label htmlFor="last_name" className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-2">Last Name</label>
            <input id="last_name" type="text" name="Last_Name" required className="w-full bg-slate-900/50 border border-slate-800 rounded-lg p-4 text-slate-200 focus:outline-none focus:border-cyan-600 transition-colors" placeholder="Doe" />
          </div>
          <div className="space-y-2">
            <label htmlFor="email" className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-2">Email Address</label>
            <input id="email" type="email" name="email" required className="w-full bg-slate-900/50 border border-slate-800 rounded-lg p-4 text-slate-200 focus:outline-none focus:border-cyan-600 transition-colors" placeholder="john@example.com" />
          </div>
          ...
          <div className="space-y-2 md:col-span-2">
            <label htmlFor="subject" className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-2">Subject</label>
            <input id="subject" type="text" name="Subject" required className="w-full bg-slate-900/50 border border-slate-800 rounded-lg p-4 text-slate-200 focus:outline-none focus:border-cyan-600 transition-colors" placeholder="Deep Questions" />
          </div>
          <div className="space-y-2 md:col-span-2">
            <label htmlFor="message" className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] ml-2">Message</label>
            <textarea id="message" rows="5" name="Message" required className="w-full bg-slate-900/50 border border-slate-800 rounded-lg p-4 text-slate-200 focus:outline-none focus:border-cyan-600 transition-colors resize-none" placeholder="Share your thoughts..."></textarea>
          </div>
      ...
      {/* SECTION 4: NEWSLETTER (ACCESSIBILITY & CONTRAST OPTIMIZED) */}
      <form onSubmit={onNewsletterSubmit} className="bg-cyan-950/20 border border-cyan-900/30 rounded-3xl p-8 md:p-16 text-center space-y-8">
        ...
        <div className="max-w-md mx-auto flex flex-col md:flex-row gap-4">
          <input 
            ...
            className="flex-grow bg-slate-950 border border-slate-800 rounded-full px-6 py-3 text-slate-200 focus:outline-none focus:border-cyan-500" 
            placeholder="your@email.com" 
          />
          {/* Changed bg-cyan-600 to bg-cyan-700 for better contrast score */}
          <button type="submit" className="bg-cyan-700 text-white px-8 py-3 rounded-full font-bold uppercase tracking-widest text-xs hover:bg-cyan-600 transition-all shadow-lg min-w-[140px]">
            {newsBtnText}
          </button>
        </div>
      </form>
      ...
      <footer className="pt-20 border-t border-slate-900 flex flex-col space-y-10">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <nav className="flex gap-6 text-slate-500 text-[10px] md:text-sm font-bold uppercase tracking-widest">
